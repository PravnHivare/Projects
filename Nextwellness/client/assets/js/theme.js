  ...
  document.querySelectorAll('[data-anchor] > a, [data-scroll-to]').forEach(function (anchor) {
    anchor.addEventListener('click', function (e) {
      ...
      if (targetElement) {
        ...
        window.scroll({
          top: (_utils$getData = utils.getData(el, 'offset-top')) !== null && _utils$getData !== void 0 ? _utils$getData : utils.getOffset(targetElement).top - 100,
          left: 0,
          behavior: 'smooth'
        });
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  setInterval(nextSlide, 5000); // Change image every 5 seconds (5000ms)
});

//float//
function toggleFloatMenu() {
  const floatMenu = document.querySelector('.float-menu');
  floatMenu.style.display = floatMenu.style.display === 'none' ? 'block' : 'none';
}
function showAdditionalItem(itemID) {
  // Display the additional item with the specified ID
  document.getElementById(itemID).style.display = 'block';
}

// Function to hide the additional list item
function hideAdditionalItem(itemID) {
  // Hide the additional item with the specified ID
  document.getElementById(itemID).style.display = 'none';
}
//shopoing //
let currentIndex = 1;
const totalImages = 3; // Change this to the total number of images you have
...
function changeImage(direction) {
  ...
  mainImage.src = `image${currentIndex}.jpg`;
}
